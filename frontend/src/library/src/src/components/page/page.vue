<template>
    <ul :class="simpleWrapClasses" :style="styles" v-if="simple">
        ...
        <!--</div>-->
      <!--#3058b6-->
      <span style="font-size: 15px;color: #495060;"> / {{this.barData[currentPage-1].projectName}}</span>
        <li
            :title="t('i.page.next')"
            :class="nextClasses"
            @click="next">
...
export default {
  ...
  methods: {
    changePage(page) {
      console.log('调用changePage方法，page为：', page)
      if (this.currentPage !== page) {
        this.currentPage = page
        this.$emit('update:current', page)
        this.$emit('on-change', page)
      }
      console.log('调用changePage方法，this.currentPage为：', this.currentPage)
    },
    prev() {
    ...
    keyUp(e) {
      ...
      if (key === 38) {
        ...
        let index
        index = this.barData.findIndex(barData => barData.projectName === e.target.value)
        // const val = parseInt(e.target.value)
        const val = index + 1
        let page = 1
