...
export
type
LayoutItemRequired = {w: number, h: number, x: number, y: number, i: string}
export
type
LayoutItem = LayoutItemRequired &
  {
...
export
type
DragEvent = {e: Event} & DragCallbackData
export
type
Size = {width: number, height: number}
export
type
ResizeEvent = {e: Event, node: HTMLElement, size: Size}
 ...
 */
export function sortIndex(layout: Layout) {
...
export function moveElement(layout: Layout, l: LayoutItem, x: Number, y: Number, isUserAction: Boolean): Layout {
  ...
  for (let i = 0; i < layout.length; i++) {
    if (x + 2 >= (layout[i].x) && x + 2 < (layout[i].x + layout[i].w) && y + 2 >= layout[i].y && y + 2 < layout[i].y + layout[i].h) {
      tempNew = i
    }
    if (il === layout[i].i) {
      tempOld = i
  ...
  if ((Math.abs(yl - layout[tempNew].y) - hl === 0) && (Math.abs(xl - layout[tempNew].x) - wl !== 0) && tempNew !== -1) {
    layout[tempOld].x = layout[tempNew].x
    layout[tempOld].y = layout[tempNew].y
    layout[tempOld].w = layout[tempNew].w
    layout[tempOld].h = layout[tempNew].h
    layout[tempNew].x = xl
    layout[tempNew].y = yl
    layout[tempNew].w = wl
    layout[tempNew].h = hl
  }
  else if ((Math.abs(yl - layout[tempNew].y) - hl !== 0) && (Math.abs(xl - layout[tempNew].x) - wl === 0) && tempNew !== -1) {
    layout[tempOld].x = layout[tempNew].x
    layout[tempOld].y = layout[tempNew].y
    layout[tempOld].w = layout[tempNew].w
    layout[tempOld].h = layout[tempNew].h
